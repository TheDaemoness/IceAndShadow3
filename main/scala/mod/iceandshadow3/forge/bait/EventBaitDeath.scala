    ...
    owner.items().foreach(item => {
      ...
      if(logicpair != null) {
        logicpair.logic.getEventFish[IEventFishItemDeath](logicpair.variant).foreach(fish => {
          item.forStateData(logicpair, state => {
            val cancel = if (event.getSource == DamageSource.OUT_OF_WORLD) {
              fish.onOwnerVoided(logicpair.variant, state, item, event.isCanceled)
            } else fish.onOwnerDeath(logicpair.variant, state, item, event.isCanceled)
