...
object WorldGenLayerTerrain {
	val varHeight = new Var[Float]("height") {
...
abstract class WorldGenLayerTerrain[Column <: TWorldGenColumnFn: ClassTag] extends TWorldGenLayer[Column] {
	private type Region = WorldGenLayerTerrain.Region[Column]
		...
		build(
			new CacheLoader[TupleXZ, Region] {
				override def load(key: TupleXZ) = new Region(
					WorldGenLayerTerrain.toEdge(key.x),
					WorldGenLayerTerrain.toEdge(key.z),
					newGenerator
				)
			}
		)

	import WorldGenLayerTerrain.remap
	...
	override def getForRegion(xFrom: Int, zFrom: Int, xWidth: Int, zWidth: Int) = {
		...
		while (xiR <= xiRMax) {
			while (ziR <= ziRMax) {
				...
				val ziBStart = ziB
				import WorldGenLayerTerrain.width
