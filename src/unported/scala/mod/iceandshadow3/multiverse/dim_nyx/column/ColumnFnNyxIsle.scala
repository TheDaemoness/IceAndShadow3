...
import mod.iceandshadow3.lib.gen.{WorldGenLayerTerrain, WorldGenColumn}
...
abstract class ColumnFnNyxIsle(cell: Cells.Result)
extends ColumnFnNyx(cell, true) {
	...
	override def apply(out: WorldGenColumn): Unit = {
		out.update(WorldGenLayerTerrain.varHeight, height)
		if(height < 48) return
