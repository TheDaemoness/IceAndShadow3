...
abstract class BEventBaitOwner[Event <: LivingEvent: ClassTag](bus: IEventBus) extends BEventBait[Event](bus) {
  ...
  override protected def handle(event: Event): Unit = {
    val owner: CRefLiving = CRefEntity.wrap(event.getEntityLiving)
    owner.items().foreach(item => {
      val logicpair = item.getLogicPair
      if(logicpair != null) {
        catchFish(logicpair).foreach(fish => {
          val triad = item.toLogicTriad(logicpair)
          item.forStateData(triad, ()=> handleFish(event, item, triad, fish) )
        })
      }
    })
  }
