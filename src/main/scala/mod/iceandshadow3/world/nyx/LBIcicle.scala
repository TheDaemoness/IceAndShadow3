...
import mod.iceandshadow3.compat.block.{AdjacentBlocks, WRefBlock}
...
class LBIcicle extends BLogicBlockSimple(DomainNyx, "icicles", MatIcicle) {
	...
	override def harvestOverride(variant: Int, block: WRefBlock, fortune: Int) =
		Array(WRefItem.make(DomainNyx.li_icicle, 0).changeCount(block.rng(2, 2+fortune)))

	override def canBeAt(block: WRefBlock, preexisting: Boolean) = {
	...
	override def onInside(block: WRefBlock, who: WEntity): Unit = {
